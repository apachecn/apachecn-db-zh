...
这两个主要部分讨论了如何使用 Memcached 和 ProxySQL 进行缓存。Memcached 是一个内存中的键值存储，可以在应用中使用，也可以使用 MySQL 中包含的特殊版本，该版本允许您直接与 InnoDB 交互。ProxySQL 结合了路由器和缓存机制，根据您定义的查询规则透明地存储结果集。
