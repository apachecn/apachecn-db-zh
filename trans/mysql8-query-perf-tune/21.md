...
InnoDB monitor 是 InnoDB information 的一种瑞士军刀，也包含有关交易的信息。InnoDB 监控器输出中的`TRANSACTIONS`部分专用于事务信息。该信息不仅包括交易列表，还包括历史列表长度。清单 [21-2](#PC5) 显示了 InnoDB monitor 的一个摘录，其中的事务部分的示例取自`INNODB_TRX`表的上一个输出之后。
