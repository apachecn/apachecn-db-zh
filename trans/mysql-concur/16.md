...
主要使用 InnoDB 监视器输出的`LATEST DETECTED DEADLOCK`部分来调查死锁。由此可以看出涉及到哪些连接、它们执行的最后一条语句、它们持有的锁以及它们正在等待的锁。此外，带有语句历史的性能模式表用于查找事务中涉及的确切语句；但是，通常情况下，您没有这种能力，必须分析应用来确定所涉及的语句。
