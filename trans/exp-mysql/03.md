...
![image](img/sq.jpg) **注意**虽然在使用配置控制下的源代码(BitKeeper)时这种技术没有被禁止，但通常是不鼓励的。事实上，开发人员以后可能会完全删除你的评论。仅当您做出不打算与任何人共享的更改时，才使用此技巧。
...
![image](img/sq.jpg) **提示**关于设置复杂条件的更多信息，参见`http://www.cmake.org/cmake/help/documentation.html`。您还可以使用`cmake-gui`应用程序通过图形界面设置选项。你可以从 cmake.org 网站下载`cmake`和`cmake-gui`。在线参考手册还包含一些使用条件编译 MySQL 的好例子。
