...
![image](images/sq.jpg) **警告** Windows 用户必须从库中删除网络 UDF。Windows 不直接支持这些功能。如果遇到关于缺少头文件或外部函数的错误，请注释掉这些函数。
...
![image](images/sq.jpg) **注意**使用`SHOW VARIABLES LIKE 'plugin%';`发现插件目录。
...
![image](images/sq.jpg) **注意**我们显示了用椭圆圈出的宏，以指示这些语句应该放在哪里。
...
![image](images/sq.jpg) **注**一些复杂的儒略历方法计算从开始日期(通常是在 18 或 19 世纪)起经过的天数。此方法假设需要儒略日/年值。
...
![image](images/sq.jpg) **注**闰年的计算故意显得幼稚。我给你留了一个更正确的计算方法作为练习。提示:jdate 变量应该在什么时候递增？
...
![image](images/sq.jpg) **注意**文件位于源代码树根下的`/sql`目录中。
...
![image](images/sq.jpg) **小心**别忘了原来`SHOW`语句前的`|`。
...
![image](images/sq.jpg) **注意**在 Windows 上，您可能需要在`my_malloc()`调用中用`MAX_PATH`代替`PATH_MAX`，用`strnicmp`代替`strncasecmp`。
...
![image](images/sq.jpg) **注意**在 Windows 上，用`MAX_PATH`代替`my_malloc()`调用中的`PATH_MAX`，用`strnicmp`代替`strncasecmp`。
