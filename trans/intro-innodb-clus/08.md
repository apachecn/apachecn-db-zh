...
构建高可用性应用需要解决与典型的多用户应用相同的所有问题。让高可用性应用变得更加困难的是，当服务器离线时，必须处理故障。幸运的是，InnoDB 集群与 MySQL 路由的结合使这变得更加容易。事实上，最大的挑战是构建到数据库(集群)的连接，以便在出现错误时重试。精明的企业应用开发人员会告诉你这是他们一直在做的事情。
