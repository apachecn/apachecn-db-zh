...
如果您计划使用单个终端，建议将输出重定向到名为`master_log.txt`的文件，并使用选项在另一个进程中启动应用程序(例如，`&`符号)。服务器生成消息时会更新日志文件，因此如果遇到问题，您可以参考这些日志文件。这也有助于保持您的终端会话清除额外的消息。下面显示了如何构建上述命令，使其作为单独的进程启动，并将消息记录到文件中:
