...
安装的软件包括 15 个应用(在 Microsoft Windows 下是 14 个),它们将与您的 MongoDB 数据库一起使用。两个“最重要”的应用是`mongo`和`mongod`应用。`mongo`应用允许您使用数据库外壳；这个 shell 使您能够用 MongoDB 完成几乎任何您想做的事情。
