...
当开始使用 MongoDB 时，大多数开发人员将从他们熟悉的来自其他数据库的基本 CRUD 操作(创建-读取-更新-删除)开始。`insert`、`find`、`update`和`delete`操作确实将构成大多数应用程序的主干。然而，在几乎所有的应用程序中，复杂的数据检索和操作需求将会存在，这超出了基本 MongoDB 命令的能力范围。

MongoDB `find()`命令功能多样且易于使用，但是聚合框架允许您将其提升到下一个级别。聚合管道可以做任何`find()`操作可以做的事情，甚至更多。正如 MongoDB 自己喜欢在博客、营销材料甚至 t 恤上说的那样:*聚合是新发现*。

聚合管道通过减少可能需要多次查找操作和复杂数据操作的逻辑来简化应用程序代码。如果利用得当，单个聚合可以取代许多查询及其相关的网络往返时间。

您可能还记得前面的章节，调优应用程序的一个重要部分是确保尽可能多的工作发生在数据库上。聚合允许您将通常位于应用程序中的数据转换逻辑移动到数据库中。因此，经过适当调整的聚合管道可以大大超越替代解决方案。
...
![../images/499970_1_En_7_Chapter/499970_1_En_7_Fig1_HTML.png](../images/499970_1_En_7_Chapter/499970_1_En_7_Fig1_HTML.png)
...
![../images/499970_1_En_7_Chapter/499970_1_En_7_Fig2_HTML.png](../images/499970_1_En_7_Chapter/499970_1_En_7_Fig2_HTML.png)
...
![../images/499970_1_En_7_Chapter/499970_1_En_7_Fig3_HTML.png](../images/499970_1_En_7_Chapter/499970_1_En_7_Fig3_HTML.png)
...
Neo4J 等图形数据库专门遍历关系图——就像你可能在社交网络中找到的那些关系图。许多非图形数据库已经整合了图形计算引擎来执行类似的任务。使用旧版本的 MongoDB，您可能不得不通过网络获取大量的图形数据，并在应用程序级别上运行一些计算。这个过程将会是缓慢而繁琐的。幸运的是，从 MongoDB 3.4 开始，我们可以使用`$graphLookup`聚合框架阶段执行简单的图遍历。
...
![../images/499970_1_En_7_Chapter/499970_1_En_7_Fig4_HTML.png](../images/499970_1_En_7_Chapter/499970_1_En_7_Fig4_HTML.png)
...
在 MongoDB 中执行聚合时，有两个重要的限制需要记住，这两个限制适用于所有聚合，不管管道是从哪个阶段构建的。除此之外，在调优您的应用程序时，还需要考虑一些特定的限制。您必须牢记的两个限制是文档大小限制和内存使用限制。
...
![../images/499970_1_En_7_Chapter/499970_1_En_7_Fig5_HTML.png](../images/499970_1_En_7_Chapter/499970_1_En_7_Fig5_HTML.png)
...
![../images/499970_1_En_7_Chapter/499970_1_En_7_Fig6_HTML.png](../images/499970_1_En_7_Chapter/499970_1_En_7_Fig6_HTML.png)

图 7-6

聚合中的磁盘排序与内存排序

使用磁盘排序要考虑的另一个问题是可伸缩性。如果您设置了`diskUsage:true`，那么您可以放心，即使没有足够的内存来完成排序，您的查询也会运行。但是，当查询从内存排序切换到磁盘排序时，性能会突然下降。在生产环境中，您的应用程序可能会突然“碰壁”

图 [7-7](#Fig7) 显示了当有足够的内存支持排序时，与相对线性的趋势相比，切换到磁盘排序如何导致执行时间的突然增加。

![../images/499970_1_En_7_Chapter/499970_1_En_7_Fig7_HTML.png](../images/499970_1_En_7_Chapter/499970_1_En_7_Fig7_HTML.png)
...
创建视图的主要优点是通过在数据库中存储复杂的管道定义来简化和统一应用程序逻辑。
...
![../images/499970_1_En_7_Chapter/499970_1_En_7_Fig8_HTML.png](../images/499970_1_En_7_Chapter/499970_1_En_7_Fig8_HTML.png)

图 7-8

物化视图与直接视图

这种方法有一个明显的缺点:当原始集合中的第二个数据发生变化时，物化视图就会过时。应用程序或数据库管理员有责任确保物化视图以有意义的时间间隔刷新。例如，实体化视图可能包含前一天的 access 销售记录。聚合可以在每晚午夜运行，确保每天的数据都是正确的。
...
MongoDB 创建了一个非常强大的方法来用聚合框架构造复杂的查询。多年来，他们扩展了这个框架，以支持更广泛的用例，甚至负责一些以前可能发生在应用程序级别的数据转换。如果从过去可以看出，`aggregate`命令将随着时间的推移而增长，以适应越来越复杂的功能。记住所有这些，如果您希望创建一个高级的高性能 MongoDB 应用程序，您应该利用`aggregate`所提供的一切。
