...
我们已经简要了解了用于连接和使用 MySQL 服务器的新 MySQL Shell、用于配置 InnoDB 集群的 AdminAPI 以及用于访问数据的 X DevAPI。然而，MySQL 中还有一个已经存在了几十年的客户端。它是一个名为`mysql`的应用，使您能够连接到服务器并在其上运行 SQL 命令。有趣的是，这个 MySQL 客户端最初被命名为 MySQL monitor，但长期以来一直被简单地称为“MySQL 客户端”、“终端监视器”，甚至是 MySQL 命令窗口。
