...
除了这些产品之外，向 MySQL 客户端添加脚本语言的请求还没有得到回应。也就是说，直到现在。然而，Oracle 并没有重组现有的(并且相当长寿的)MySQL 客户端工具，而是发布了一个名为 MySQL Shell 的新客户端，它支持脚本语言、X DevAPI 以及 SQL 命令等等。但是新的Shell远不止这些。
...
可以使用几个控制模式、连接、行为等的启动选项来启动 shell。本节介绍一些您可能想要使用的更常用的选项。我们将在后面的章节中看到更多关于连接选项的内容。表 [4-2](#Tab2) 显示了常见Shell选项的列表。
